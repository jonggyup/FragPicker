...
/usr/share/bcc/tools/trace -n $1 'vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos) "ino = %llu | size = %llu | pos = %u | direct = %d | type = 0", file->f_inode->i_ino, count, *pos, file->f_flags & O_DIRECT' >> ./trace.result &
