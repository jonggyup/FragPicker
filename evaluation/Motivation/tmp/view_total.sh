...
# $1 frag_unit $2 distance $3 request size
array=(read_perf)
#trim_time.result
...
if [ "$var1" == "frag" ]; then	
	...
	#echo $tmp1
	value=$(cat ./results_tmp/$1/read_perf_0_0_$4.result | awk '{print $3}')

fi
printf "%-8.7s%7s" "$value" "  |  " 
printf "\n"

#filename = read_perf_$fragunit_$ditance_$rasize.result
		...
		do
			...
			do
				...
				if [ "$var1" == "frag" ]; then	
					...
					tmp1=$(echo $i | cut -d. -f 1)
					#echo $tmp1gg
					value=$(cat $base_dir/${i}_${2}_${3}_$4.result | awk '{print $3}')
